...
- [ ] Hero words ("Launching lifelong kingdom workers from every college town.") reveal in staggered sequence on load.
...
- [ ] Confirm exactly one `<h1>` per page (spot-checked in source: `index.astro` "Welcome to Silicon Valley College Church", `about.astro` "Launching lifelong kingdom workers...", `gather.astro` "SVC Sundays", `connect.astro` "Connect", `campuses/index.astro` "Campuses", each campus page's school name, `class-of-2030.astro` "Class of 2030", `404.astro` "Looks like this page wandered off").
